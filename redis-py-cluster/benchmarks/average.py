...
def timeit_average(rc,num,average):
    """
    Time how long it take to run a number of set/get:s
    """
    start=time.time()
    ...
    for i in range(0,num):  # noqa
        ...
        if time.time()>=start+existing+1:
            ...
            for i in range(len(times)):
                all_time+=times[i]
            average[existing]=all_time/float(operation-last)
            times=[]
            existing+=1
            last=operation
    
def timeit_pipeline_average(rc, num,average):
    """
...
if __name__ == "__main__":
    ...
    for j in range(int(args["-c"])-1):
        ...
        processes.append(p)
    if args["--pipeline"]:
        p = multiprocessing.Process(target=timeit_pipeline_average, args=(rc, single_request,average))
    else:
        p = multiprocessing.Process(target=timeit_average, args=(rc, single_request,average))
    processes.append(p)
    t1=time.time()
    for p in processes:
        p.start()
